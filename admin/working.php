...
$toinsert = $result->fetch_assoc();
if ($toinsert) {
    $file = $toinsert['file'];
    $title = $toinsert['tid'];
    $uid = $toinsert['uid'];
    $date = $toinsert['issue_date'];
    $issue_date = (new DateTime())->format('Y-m-d');
    $comment = $toinsert['comment'];
} else {
    $file = '';
    $title = '';
    $uid = '';
    $date = '';  // Set $date to an empty string or any default value you want.
    $issue_date = (new DateTime())->format('Y-m-d');
    $comment = '';
}
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['issue_id'])) {
        ...
        $conn->query("UPDATE issue SET sid = 5  ,completion_date = '$issue_date'  WHERE issue_id = $issue_id");
        $result = $conn->query("INSERT INTO `history` (`tid`, `uid`, `issue_id`,`issue_date`, `completion_date`, `filepath`,`status`) VALUES ('$title', '$uid', '$issue_id' , '$date' , '$issue_date', '$file',1);");
        if (!$result) {
...
<body>
    ...
    <main class="main-content position-relative border-radius-lg ">
        <div class="container ml-5">
            <br>
            <br>
            <h3 style="color: white;">รายการคำร้องที่กำลังดำเนินงาน</h3>
            <div>
...
");
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<td>
                                    <a href="' . $_SERVER['PHP_SELF'] . '?issue_id=' . $row["issue_id"] . '" class="btn btn-success ml-2">ดำเนินการเสร็จสิ้น</a>
                                    <a href="javascript:void(0);" class="btn btn-warning" onclick="showCommentForm(' . $row["issue_id"] . ')">ต้องแก้ไข</a>
                                </td>';
                                echo "</tr>";
                            }
                        } else
