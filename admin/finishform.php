...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['issue_id'])) {
        ...
        $conn->query("UPDATE issue SET sid = 5 WHERE issue_id = $issue_id");
        $result = $conn->query("INSERT INTO `history` (`tid`, `uid`, `issue_id`, `completion_date`, `filepath`) VALUES ('$title', '$uid', '$issue_id', '$issue_date', '$file');");
        if (!$result) {
...
<body>
    ...
    <main class="main-content position-relative border-radius-lg ">
        <div class="container ml-5">
            <br>
            <br>
            <h3 style="color: white;">รายการคำร้องที่ดำเนินงานเสร็จสิ้น</h3>
            <div>
                <table class="table table-bordered bg-light">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th>วันที่เสร็จสิ้นคำร้อง</th>
                            <th>ไฟล์ที่แนบมากับยื่นคำร้อง</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $result = $conn->query("SELECT h.issue_id , t.title,u.name ,h.filepath,h.issue_date, h.completion_date FROM history h , users u , title t Where h.uid = '2' AND h.uid = u.uid AND h.tid = t.tid AND status = 1;");
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<td><a href="' . $row["filepath"] . '">ดูเอกสาร</a></td>';
                                echo "</tr>";
                            }
                        } else
                        $conn->close();
