...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    if(isset($_GET['issue_id'])){
        ...
        $conn -> query("UPDATE issue SET sid = 3 WHERE issue_id = $issue_id");
...
<main class="main-content position-relative border-radius-lg ">
    <div class="container  ml-5">
        <br>
        <br>
        <h3 style="color: white;">รายการคำร้องที่กำลังตรวจสอบ</h3>
        <div>
            <table class="table table-bordered bg-light">
                ...
                <tbody>
                    <?php
                    $result = $conn->query("SELECT ss.issue_id,  ss.sid,t.title, u.name, ss.issue_date, 
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<td>
                                    <a href="' . $_SERVER['PHP_SELF'] . '?issue_id=' . $row["issue_id"] . '" class="btn btn-success ml-2">รับคำร้อง</a>
                                    <a href="javascript:void(0);" class="btn btn-warning" onclick="showCommentForm(' . $row["issue_id"] . ')">ไม่รับคำร้อง</a>
                                </td>';
                            echo "</tr>";
                        }
                    }
